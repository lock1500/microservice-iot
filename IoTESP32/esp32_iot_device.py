...
class ESP32Device:
    ...
    def get_api_base_url(self):
        return f"http://{config.ESP32_DEVICE_HOST}:{config.ESP32_DEVICE_PORT}"
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting Flask API for ESP32Device on port {config.ESP32_API_PORT}")
    app.run(host="0.0.0.0", port=config.ESP32_API_PORT, threaded=True)